...
with tempfile.TemporaryDirectory() as directory:
    ...
    with sqlite3.connect(database) as connection:
        ...
        if connection.execute("PRAGMA user_version").fetchone()[0] != 26:
            raise SystemExit("fresh migration did not reach schema 26")
        if connection.execute("SELECT MAX(version) FROM schema_migrations").fetchone()[0] != 26:
            raise SystemExit("fresh migration ledger did not reach 26")
