...
CLONES_ROOT = ROOT / "workspaces/.orchestra-worker-clones"
...
def _runtime_user() -> str:
    raw_uid = os.environ.get("ORCHESTRA_UID", str(os.getuid()))
    raw_gid = os.environ.get("ORCHESTRA_GID", str(os.getgid()))
    if re.fullmatch(r"(?:0|[1-9][0-9]*)", raw_uid) is None or re.fullmatch(
        r"(?:0|[1-9][0-9]*)", raw_gid
    ) is None:
        raise RuntimeError("Orchestra runtime UID/GID authority is invalid")
    return f"{raw_uid}:{raw_gid}"
...
def nested_docker(
    *arguments: str,
    ...
    return run_command(
        ["docker", *arguments],
        check=check,
        env=PRIVATE_DOCKER_ENVIRONMENT,
    )
...
def verify_worktree(run: sqlite3.Row) -> tuple[Path, Path]:
    ...
    managed_root = (
        ROOT / "workspaces/.orchestra-worktrees"
    ).resolve()
...
def prepare_worker_clone(
    *,
    ...
    git(
        ...
        "config",
        "user.name",
        "Orchestra Controlled Worker",
    )
    git(
        clone,
        "config",
        "user.email",
        "worker@orchestra.local",
    )
...
def authorized_sandbox_container_id(
    ...
    *,
    ...
    if (
        ...
        or labels.get("orchestra-task-id") != task_id
...
def precreate_worker_sandbox(
    *,
    ...
    created = nested_docker(
        ...
        "orchestra-sandbox=1",
        "--label",
        f"orchestra-task-id={task_id}",
        "--label",
        f"orchestra-runtime-request-id={runtime_request_id}",
        "--network=none",
        "--user",
        RUNTIME_USER,
        "--cpus",
...
def audit_sandbox(
    *,
    ...
    if labels.get("orchestra-task-id") != task_id:
        fail("Sandbox task binding mismatch")
    if labels.get("orchestra-runtime-request-id") != runtime_request_id:
...
def command_launch(
    ...
    try:
        ...
        sandbox_id, audit, preflight = precreate_worker_sandbox(
            container_name=f"orchestra-sandbox-{suffix}",
            task_id=task_id,
            runtime_request_id=runtime_request_id,
            clone=clone,
            prepared_environment=prepared_environment,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Orchestra controlled worker launcher"
    )
