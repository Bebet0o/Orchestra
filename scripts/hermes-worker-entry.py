...
sandbox_handle = os.environ.get("HERMESOPS_SANDBOX_HANDLE", "").strip()
...
cpu_value = os.environ.get("HERMESOPS_SANDBOX_CPU_LIMIT", "").strip()
memory_value = os.environ.get("HERMESOPS_SANDBOX_MEMORY_MB", "").strip()
network_value = os.environ.get("TERMINAL_DOCKER_NETWORK", "").strip().lower()

if not re.fullmatch(r"[a-f0-9]{64}", sandbox_handle):
    raise RuntimeError("HermesOps sandbox handle is invalid")

if not re.fullmatch(r"task-[a-f0-9]{32}", task_id):
    raise RuntimeError("HermesOps sandbox task identity is invalid")
...
if read_only_value not in {"true", "false"}:
    raise RuntimeError("HermesOps sandbox mount policy is invalid")

try:
    cpu_limit = int(cpu_value)
    memory_mb = int(memory_value)
except ValueError as error:
    raise RuntimeError("HermesOps sandbox resource policy is invalid") from error

if cpu_limit <= 0 or memory_mb <= 0:
    raise RuntimeError("HermesOps sandbox resource policy is invalid")
...
def _validate_authorized_sandbox(
    ...
    *,
    ...
    if container.get("Id") != expected_handle:
        raise RuntimeError("Authorized HermesOps sandbox identity changed")
    if labels.get("hermesops-sandbox") != "1":
        raise RuntimeError("Authorized HermesOps sandbox owner is mismatched")
    if labels.get("hermesops-task-id") != expected_task_id:
        raise RuntimeError("Authorized HermesOps sandbox task is mismatched")
    if labels.get("hermesops-runtime-request-id") != expected_request_id:
    ...
    if len(workspace_mounts) != 1:
        raise RuntimeError("Authorized HermesOps workspace mount is invalid")
    ...
    if (
        ...
        raise RuntimeError("Authorized HermesOps workspace policy is mismatched")
    for candidate in (container.get("Mounts") or []):
        ...
        if (
            ...
            raise RuntimeError("Authorized HermesOps mount set is mismatched")
    ...
    if expected_network_enabled:
        if actual_network_mode == "none" or not effective_networks:
            raise RuntimeError("Authorized HermesOps network policy is mismatched")
    elif actual_network_mode != "none" or effective_networks:
        raise RuntimeError("Authorized HermesOps network policy is mismatched")

    if int(host_config.get("NanoCpus") or 0) != expected_cpu_limit * 1_000_000_000:
        raise RuntimeError("Authorized HermesOps CPU policy is mismatched")
    if int(host_config.get("Memory") or 0) != expected_memory_mb * 1024 * 1024:
        raise RuntimeError("Authorized HermesOps memory policy is mismatched")
    if int(host_config.get("PidsLimit") or 0) != 256:
        raise RuntimeError("Authorized HermesOps PID policy is mismatched")
    if bool(host_config.get("Privileged")):
        raise RuntimeError("Authorized HermesOps sandbox is privileged")
    ...
    if not any(
        ...
        raise RuntimeError("Authorized HermesOps security policy is mismatched")
    cap_drop = {
        str(value).upper() for value in (host_config.get("CapDrop") or [])
    }
    if "ALL" not in cap_drop:
        raise RuntimeError("Authorized HermesOps capability policy is mismatched")
    if str(config.get("User") or "") not in {"1000", "1000:1000"}:
        raise RuntimeError("Authorized HermesOps user policy is mismatched")
    sensitive_fragments = (
        "OPENAI",
        "CODEX",
        "API_SERVER_KEY",
    ...
    if any(
        ...
        raise RuntimeError("Authorized HermesOps environment is sensitive")
...
def _find_authorized_sandbox(
    ...
    if inspected.returncode != 0:
        raise RuntimeError("Authorized HermesOps sandbox is unavailable")

    payload = json.loads(inspected.stdout)
    if not isinstance(payload, list) or len(payload) != 1:
        raise RuntimeError("Authorized HermesOps sandbox inspection is invalid")
    container = payload[0]
    if _task_label != task_id:
        raise RuntimeError("HermesOps terminal task identity is mismatched")
    if _profile_label != profile_name:
        raise RuntimeError("HermesOps terminal profile identity is mismatched")
...
if effective.get("docker_network") is not network_enabled:
    raise RuntimeError("HermesOps sandbox network policy is not active")
